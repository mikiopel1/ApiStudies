...
public class ApiController {
    ...
    public ResponseEntity<String> createPost() {
        PostClass newPostClass = new PostClass(1L, "New Post", "This is the body of the new post.");

        HttpHeaders headers = new HttpHeaders();
        headers.set("Content-Type", "application/json");


        HttpEntity<PostClass> requestEntity = new HttpEntity<>(newPostClass, headers);

        try {
            ResponseEntity<String> responseEntity = restTemplate.postForEntity(API_URL, requestEntity, String.class);
            return new ResponseEntity<>("Utworzono post"+ newPostClass, HttpStatus.CREATED);
